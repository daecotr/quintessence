...
int main(int argc, char **argv) {
	try {
		...
		std::vector<const char *> instanceLayers = {"VK_LAYER_KHRONOS_validation"},
															instanceExtensions = {vk::EXTDebugUtilsExtensionName};
		...
		for (const auto &instanceLayer: instanceLayers) {
			...
			if (!instanceLayerFound)
				throw std::runtime_error("Failed to find required instance layer " + std::string{instanceLayer});
		}
		...
		instanceExtensions.insert(instanceExtensions.end(), glfwInstanceExtensions,
															glfwInstanceExtensions + glfwInstanceExtensionCount);
		...
		for (const auto &instanceExtension: instanceExtensions) {
			...
			for (const auto &availableInstanceExtension: availableInstanceExtensions) {
				if (std::strcmp(availableInstanceExtension.extensionName, instanceExtension) == 0) {
					instanceExtensionFound = true;
					break;
				}
			}
			if (!instanceExtensionFound)
				throw std::runtime_error("Failed to find required instance extension " + std::string{instanceExtension});
		}
		...
		constexpr vk::DebugUtilsMessengerCreateInfoEXT debugUtilsMessengerCreateInfo{
				...
				[](VkDebugUtilsMessageSeverityFlagBitsEXT, VkDebugUtilsMessageTypeFlagsEXT,
					 const VkDebugUtilsMessengerCallbackDataEXT *pCallbackData, void *) VKAPI_ATTR -> vk::Bool32 {
					std::cout << pCallbackData->pMessage << std::endl;
					return vk::False;
				}};

		const vk::DispatchLoaderDynamic dispatchLoaderDynamic{*instance, vkGetInstanceProcAddr};
		auto debugUtilsMessenger =
				instance->createDebugUtilsMessengerEXTUnique(debugUtilsMessengerCreateInfo, nullptr, dispatchLoaderDynamic);
